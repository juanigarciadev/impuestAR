...
export default function Calculator() {
    ...
    const iva = useMemo(()=> Math.round(value*precioDolar*0.21), [value, precioDolar])
    const ganancias = useMemo(()=> Math.round(value*precioDolar*0.30), [value, precioDolar])
        ...
        <section className="flex flex-col gap-4 w-full">
            <div className="w-full py-2 px-6 border border-neutral-300 rounded-lg cursor-pointer hover:bg-neutral-100 duration-200 relative" onClick={()=>setDropdownToggle(!dropdownToggle)}>
                ...
                {dropdownToggle ? <div className="absolute bg-white w-full top-12 left-0 border border-neutral-300 shadow-sm rounded-lg">
                    <div className="w-full hover:bg-neutral-100 py-4 px-6 select-none" onClick={()=>updateCurrency('Dólar Tarjeta', precioDolarInicial)}>
                        <span className="text-sm text-neutral-500">Dólar Tarjeta</span>
                    </div>
                    <div className="w-full hover:bg-neutral-100 py-4 px-6 select-none" onClick={()=>setDropdownTitle('Dólar MEP')}>
                        <span className="text-sm text-neutral-500">Dólar MEP</span>
                    </div>
                    <div className="w-full hover:bg-neutral-100 py-4 px-6 select-none" onClick={()=>updateCurrency('Dólar Crypto', precioDolarCrypto)}>
                        <span className="text-sm text-neutral-500">Dólar Crypto</span>
                    </div>
                    <div className="w-full hover:bg-neutral-100 py-4 px-6 select-none" onClick={()=>updateCurrency('Dólar Blue', precioDolarBlue)}>
